...
function checkReUserName(flag){
    $.ajax({
        url: "http://119.3.170.239:9400/xzzj/bbs/account/checkUserName",
        type: "POST",
...
$("input[name='email']").blur(function () {
    if(!email_check($("input[name='email']"),$(".checkEmail"))){
        var url = "http://119.3.170.239:9400/xzzj/bbs/account/checkEmail";
...
$("input[name='forgetEmail']").blur(function () {
    if(!email_check($("input[name='forgetEmail']"),$(".checkForgetEmail"))){
        var url = "http://119.3.170.239:9400/xzzj/bbs/account/emailExists";
...
function checkCode(flag, codeCache, $inputCode, $inputEmail, $checkTip){
    $.ajax({
        url: "http://119.3.170.239:9400/xzzj/bbs/account/checkCode",
        type: "POST",
...
$(".get-code").click(function () {
    ...
    $.ajax({
        url: "http://119.3.170.239:9400/xzzj/bbs/account/getCode",
        type: "GET",
...
$('#login-form input[type="submit"]').click(function (e) {
    ...
    if(forget){
        var flag = false;
        $.ajax({
            url: "http://119.3.170.239:9400/xzzj/bbs/account/forgetPassword",
            type: "POST",
            data:
                {
    ...
    if(register){
        ...
        if(reUserName && reCode && reEmail){
            if(submit("http://119.3.170.239:9400/xzzj/bbs/account/register")){
                setTimeout(function () {
                    window.location.replace("http://119.3.170.239:9400/xzzj/login");
                }, 3000)
            }
        ...
        if(submit("http://119.3.170.239:9400/xzzj/bbs/account/login")) {
            window.location.replace("http://119.3.170.239:9400/xzzj/xzzj_home");
        }
    }
});